...
#define CMRI_NODE_DESCRIPTION  "BBLeo BASE_NODE_8IN8OUT + I2C"
...
ioMap *setupMap() {
    ...
    //    The choice of HOST_CENTRIC or NODE_CENTRIC will determine whether the output on pin 12 is
    //    referenced as CT0001 (HOST_CENTRIC) or CT0009 (NODE_CENTRIC)...

    iomap->add(INPUT,   BUILTIN,         4,   INPUT_PULLUP          ); //  Detector Staging Entry
    iomap->add(INPUT,   BUILTIN,         5,   INPUT        | INVERT ); //  Detector Yard Track 1 entry
    iomap->add(INPUT,   BUILTIN,         6,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 1 mid
    iomap->add(INPUT,   BUILTIN,         7,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 1 exit
    iomap->add(INPUT,   BUILTIN,         8,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 2 entry
    iomap->add(INPUT,   BUILTIN,         9,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 2 mid
    iomap->add(INPUT,   BUILTIN,        10,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 2 exit
    iomap->add(INPUT,   BUILTIN,        11,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 3 entry

    iomap->add(OUTPUT,  BUILTIN,        12,   OUTPUT_LOW   | INVERT ); //  Detector Yard Track 3 mid
    iomap->add(OUTPUT,  BUILTIN,        13,   OUTPUT_LOW   | INVERT ); //  Detector Yard Track 3 exit
    iomap->add(OUTPUT,  BUILTIN,        A0,   OUTPUT_LOW   | INVERT ); //  Detector Yard Track 4 entry
    iomap->add(OUTPUT,  BUILTIN,        A1,   OUTPUT_LOW   | INVERT ); //  Detector Yard Track 4 mid
    iomap->add(OUTPUT,  BUILTIN,        A2,   OUTPUT_LOW   | INVERT ); //  Detector Yard Track 4 exit
    iomap->add(OUTPUT,  BUILTIN,        A3,   OUTPUT_HIGH           ); //  LED - Yardmaster call alert
    iomap->add(OUTPUT,  BUILTIN,        A4,   OUTPUT_HIGH           ); //  Audio Trigger - Train Entering Station
    iomap->add(OUTPUT,  BUILTIN,        A5,   OUTPUT_HIGH           ); //  Audio Trigger - Train Leaving Station
