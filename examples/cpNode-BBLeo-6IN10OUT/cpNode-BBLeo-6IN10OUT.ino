...
#define CMRI_NODE_DESCRIPTION  "BBLeo BASE_NODE_6IN10OUT + I2C"
...
ioMap *setupMap() {
    ...
    //    The choice of HOST_CENTRIC or NODE_CENTRIC will determine whether the output on pin 10 is
    //    referenced as CT0001 (HOST_CENTRIC) or CT0007 (NODE_CENTRIC)...


    iomap->add(INPUT,   BUILTIN,         4,   INPUT_PULLUP          ); //  Detector Staging Entry
    iomap->add(INPUT,   BUILTIN,         5,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 1
    iomap->add(INPUT,   BUILTIN,         6,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 2
    iomap->add(INPUT,   BUILTIN,         7,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 3
    iomap->add(INPUT,   BUILTIN,         8,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 4
    iomap->add(INPUT,   BUILTIN,         9,   INPUT_PULLUP | INVERT ); //  Detector Yard Track 5
    iomap->add(OUTPUT,  BUILTIN,        10,   OUTPUT_LOW            ); //  SIG2 STOP
    iomap->add(OUTPUT,  BUILTIN,        11,   OUTPUT_LOW            ); //  SIG2 LEFT

    iomap->add(OUTPUT,  BUILTIN,        12,   OUTPUT_LOW            ); //  SIG2 RIGHT
    iomap->add(OUTPUT,  BUILTIN,        13,   OUTPUT_LOW   | INVERT ); //  some feature
    iomap->add(OUTPUT,  BUILTIN,        A0,   OUTPUT_LOW   | INVERT ); //  some other feature
    iomap->add(OUTPUT,  BUILTIN,        A1,   OUTPUT_LOW   | INVERT ); //  UNUSED
    iomap->add(OUTPUT,  BUILTIN,        A2,   OUTPUT_LOW   | INVERT ); //  UNUSED
