...
#define CMRI_NODE_DESCRIPTION  "BBLeo BASE_NODE_16OUT + I2C"
...
ioMap *setupMap() {
    ...
    iomap->add(OUTPUT,  BUILTIN,         4,   OUTPUT_LOW); //
    iomap->add(OUTPUT,  BUILTIN,         5,   OUTPUT_LOW); //
    iomap->add(OUTPUT,  BUILTIN,         6,   OUTPUT_LOW); //
    iomap->add(OUTPUT,  BUILTIN,         7,   OUTPUT_LOW); //
    iomap->add(OUTPUT,  BUILTIN,         8,   OUTPUT_LOW); //
    iomap->add(OUTPUT,  BUILTIN,         9,   OUTPUT_LOW); //
    iomap->add(OUTPUT,  BUILTIN,        10,   OUTPUT_LOW); //
    iomap->add(OUTPUT,  BUILTIN,        11,   OUTPUT_LOW); //

    iomap->add(OUTPUT,  BUILTIN,        12,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  BUILTIN,        13,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  BUILTIN,        A0,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  BUILTIN,        A1,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  BUILTIN,        A2,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  BUILTIN,        A3,   OUTPUT_HIGH          ); //
    iomap->add(OUTPUT,  BUILTIN,        A4,   OUTPUT_HIGH          ); //
    iomap->add(OUTPUT,  BUILTIN,        A5,   OUTPUT_HIGH          ); //

    iomap->add(OUTPUT,  mcp23017_20,     0,   OUTPUT_HIGH | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,     1,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,     2,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,     3,   OUTPUT_LOW  | INVERT ); //

    iomap->add(OUTPUT,  mcp23017_20,     4,   OUTPUT_HIGH          ); //
    iomap->add(OUTPUT,  mcp23017_20,     5,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,     6,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,     7,   OUTPUT_LOW  | INVERT ); //

    iomap->add(OUTPUT,  mcp23017_20,     8,   OUTPUT_HIGH          ); //
    iomap->add(OUTPUT,  mcp23017_20,     9,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,    10,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,    11,   OUTPUT_LOW  | INVERT ); //

    iomap->add(OUTPUT,  mcp23017_20,    12,   OUTPUT_HIGH          ); //
    iomap->add(OUTPUT,  mcp23017_20,    13,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,    14,   OUTPUT_LOW  | INVERT ); //
    iomap->add(OUTPUT,  mcp23017_20,    15,   OUTPUT_LOW  | INVERT ); //
